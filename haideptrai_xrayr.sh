...
do
 ...
 echo "---------- PUNPN CUNG CAP 4G GIA RE ----------"
 echo "--------- HANG A HAI ---------"
 echo "-------------------------------"
 echo "CHỌN GIAO THỨC CẦN CÀI - PUNPN.XYZ -"
 echo "-------------------------------"
 echo "----------- DEV MOD ----------"
  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        CertDomain: "fuzyvpn" # Domain to cert
        CertFile: /etc/XrayR/fuzzy.crt # Provided if the CertMode is file
        KeyFile: /etc/XrayR/fuzzy.key
...
case "${num}" in
1) bash <(curl -Ls https://5ggiasinhvien.store/XrayRVIP/Fastvnteam-Install-XrayR.sh)
